...
static u8 rtw_usb_get_tx_qsel(struct sk_buff *skb,
			      enum rtw_tx_queue_type queue)
{
	switch (queue) {
...
static void rtw_usb_tx_queue(struct rtw_usb *rtwusb, struct sk_buff *skb);
...
static void rtw_usb_tx_queue(struct rtw_usb *rtwusb, struct sk_buff *skb)
{
	...
	txcb = rtw_usb_txcb_init(rtwdev, queue);
	if (!txcb)
		return;

	ret = rtw_usb_write_port_tx(rtwdev, queue, skb->len, skb,
				    txcb);
	if (ret)
					...
					u32 size)
{
	...
	return rtw_usb_write_data(rtwdev, &pkt_info, buf, RTW_TX_QUEUE_BCN);
	*/
	struct rtw_chip_info *chip = rtwdev->chip;
	struct rtw_usb *rtwusb;
	struct rtw_tx_pkt_info pkt_info = {};
	...
	return rtw_usb_write_data(rtwdev, &pkt_info, skb, RTW_TX_QUEUE_BCN);
}
...
static int rtw_usb_write_data_h2c(struct rtw_dev *rtwdev, u8 *buf, u32 size)
{
	...
	return rtw_usb_write_data(rtwdev, &pkt_info, skb, RTW_TX_QUEUE_H2C);
}
